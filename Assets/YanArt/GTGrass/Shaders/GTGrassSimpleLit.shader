Shader "YanArt/GTGrassSimpleLit"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed _Power, _AO, _DiffuseIntensity, _AlphaClip;
            ...
            g2f GrassBlade(float3 pos, float3 offset, float3x3 transformMatrix, float2 uv, fixed3 color, fixed3 normal)
            {
                g2f o = (g2f)0;
            ...
            fixed4 frag(g2f i) : SV_Target
            {
                ...
                fixed dotNL = max(0,dot(N,L));
                fixed4 Diffuse = Ambient + Kd * LightColor * dotNL;
                c *= Diffuse;
                //Shadow
                UNITY_LIGHT_ATTENUATION(atten, i, i.positionWS)
                atten += _AO; // _AO is for simulating ambient light
                c *= atten;
        ...
        Pass
        {
            ...
            fixed4 frag(g2f i) : SV_Target
            {
                ...
                fixed3 N = normalize(i.normal);
                fixed3 L = normalize(_WorldSpaceLightPos0 - i.positionWS);
                fixed dotNL = max(0,dot(N,L));
                fixed4 Diffuse = LightColor * dotNL;
                c *= Diffuse * _DiffuseIntensity;
        ...
        Pass
        {
            ...
            Varyings GrassBlade(float3 pos, float3 offset, float2 uv, float3x3 transformMatrix, float3 normalOS)
            {
                Varyings o = (Varyings)0;
                float3 newPos = mul(unity_WorldToObject,float4(pos,1)).xyz + mul(transformMatrix, offset);
                o.positionWS = mul(unity_ObjectToWorld, newPos);
                o.pos = UnityClipSpaceShadowCasterPos(newPos, normalOS);
            ...
            void geo(triangle v2g IN[3], inout TriangleStream<Varyings> triStream)
            {
                ...
                for(int i = 0; i < 3; i++)
                {
                    ...
                    triStream.Append(GrassBlade(posWS, offset, float2(0,t), transformationMatrix, vNormal));
                    triStream.Append(GrassBlade(posWS, float3(-offset.x,offset.y,offset.z), float2(1,t), transformationMatrix, vNormal));
                }

                triStream.RestartStrip();
            }

            fixed4 ShadowPassFragment(Varyings input) : SV_TARGET
            {
                fixed alpha = tex2D(_BaseMap, input.uv).a;
