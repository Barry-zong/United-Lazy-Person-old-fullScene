...
void GetSurfaceAndBuiltinData(FragInputs input, float3 V, float3 N, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData RAY_TRACING_OPTIONAL_PARAMETERS)
{
...
// Don't dither if displaced tessellation (we're fading out the displacement instead to match the next LOD)
    ...
    surfaceData.normalWS = N; // GTGrass: Rather than calculating, we pass the normalWS from outside
