Shader "YanArt/GTGrassHDRPUnlit"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                half _WindStrength;
                half _TessellationFactor, _TessellationMin, _TessellationMax, _TessellationFactorMinDistance, _TessellationFactorMaxDistance;
                half3 _WindOffset;
                int _BufferCount;
            ...
            PackedVaryingsMeshToPS GrassBlade(float3 pos, float3 offset, float3x3 transformMatrix, float2 uv, half4 color)
            {
                VaryingsMeshToPS o;
            ...
            {
                float3 positionRWS = packedInput.interpolators0;
                float2 uv = packedInput.interpolators3;
                half4 color = packedInput.interpolators5;

                clip(_TessellationFactorMaxDistance - length(positionRWS));
                half4 c = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, uv);
                clip(c.a - _AlphaClip);

                c *= lerp(_BottomColor, _TopColor * color, saturate(pow(abs(uv.y), _Power)));
                return c;
            }
